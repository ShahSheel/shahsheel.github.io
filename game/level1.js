...
for (var i = 0; i < level.length; i++) {
    for (var j = 0; j < level[i].length; j++) {
           if (level[i][j] == 'd') {
            var dooropen = game.add.sprite(30+20*j, 297, 'doorlocked');
    ...
    levelState = game.add.text(game.world.width-108, 0.5, 'Level 1', { fontSize: '24px', fill: '#fff' });
...
disableLaser: function(player,buttons){
    disable = game.add.text(300, 40, 'Press Enter to disable the laser!', { fontSize: '12px'}); 
